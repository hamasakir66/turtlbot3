...
void Turtlebot3Drive::laserScanMsgCallBack(const sensor_msgs::LaserScan::ConstPtr &msg)
{
  ...
  for (int num = 0; num < 3; num++)// 3 toiunoha sensor no iti 
  {
    if (std::isinf(msg->ranges.at(scan_angle[num])))// mugenn no sei no atai ka douka wo kakuninn siteiru . at= specific angle 
    //mugenn nara sonomama (tumari mugenn nara scan_data ha mugenn = obstacle ga nai teiukoto)
    {
      scan_data_[num] = msg->range_max; //tumari kokoni obstacle ha nai toiukoto. 
    }
    else
    {
      scan_data_[num] = msg->ranges.at(scan_angle[num]); // obstacle made no kyori. 0,30,330 wo atehameta range noatai woireru. 
    }
  }
}
...
{
  static uint8_t turtlebot3_state_num = 0; //KONO KATA DE KONO HENNSUU WO SENGENN SURU

  switch(turtlebot3_state_num)
  {
    case GET_TB3_DIRECTION:
      if (scan_data_[CENTER] > check_forward_dist_)//siki iti wo 0.7 m nisiteru ga, sikiiti wokoeruka kakuninn
      {
